	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			for (final ApplicationInfo apinfo : installed) {
				...
				// filter applications which are not allowed to access the   将没有internet访问权限的应用去�?
				// Internet
				if (app == null
				...
				if (name.length() == 0) {
					...
					name = pkgmanager.getApplicationLabel(apinfo).toString();//此函数执行非常慢�?
			...
			for (int i = 0; i < special.length; i++) {
				app = special[i];
				if (app.uid != -1 && !map.containsKey(app.uid)) {  //确保目前的droidapps中没有重复的�?
					// check if this application is allowed 确保当前的应用被选中上网
...
//						Toast.LENGTH_LONG).show();
				Log.d("Tag", "the binary files has been installed!");
			}
		} catch (Exception e) {
			if (showErrors)
	...
	/**
	 * Internal thread used to execute scripts (as root or not).具体执行脚本的线程类
	 */
	private static final class ScriptRunner extends Thread {
		...
		public void run() {
			try {
				file.createNewFile();//这个函数不一定就建立�?��新文件，而是根据路径，文件存在则返回true；不存在则新建一�?
				final String abspath = file.getAbsolutePath();
				// make sure we have execution permission on the script file�?/执行chmod命令 添加权限，确保有执行脚本文件的权�?
				Runtime.getRuntime().exec("chmod 777 " + abspath).waitFor();
				// Write the script to be executed //写执行脚本内容到文件
				final OutputStreamWriter out = new OutputStreamWriter(
						new FileOutputStream(file));
				if (new File("/system/bin/sh").exists()) {    //�?��sh是否存在
					out.write("#!/system/bin/sh\n"); // 写脚本头
				}
				out.write(script);  //接着，写入脚本内容字符串
				if (!script.endsWith("\n"))
					out.write("\n");
				out.write("exit\n");
				out.flush();
				out.close();
				if (this.asroot) {   //根据root标志 执行脚本文件
					// Create the "su" request to run the script 获取ROOT权限来运行脚�?
					exec = Runtime.getRuntime().exec("su -c " + abspath);
				} else {
					// Create the "sh" request to run the script
					exec = Runtime.getRuntime().exec("sh " + abspath);
				}
				//下面应该是脚本执行响应的接收�?对res参数写入响应信息�?这应该算是个输出参数�?
